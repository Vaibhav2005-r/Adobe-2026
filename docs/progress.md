...
**1. The brief is still genuinely unavailable — but the format assumption was wrong anyway, and is now fixed.** Confirmed the brief isn't recoverable: not in `~/Downloads/claude-code-context/` (only `CLAUDE.md` + `build-plan.md`, both already in the repo), not in git history, not on any branch of the GitHub remote. The PDF provided mid-session (`round3-winning-plan.md.pdf`) turned out to be our own build plan, not the brief. So instead of continuing to rely on the build plan's *summary* of the brief, checked the **published Claude Code plugin-marketplace spec directly** (code.claude.com/docs/en/plugin-marketplaces, plus Anthropic's own `.claude-plugin/marketplace.json` and the community JSON Schema). Result: a real, previously-unflagged incompatibility — the official spec expects the manifest at **`.claude-plugin/marketplace.json`**, requires `name`/`owner`/`plugins`, and defines `metadata` as `additionalProperties: false` permitting only `description`/`version`/`pluginRoot`. **Our root manifest's `metadata.entrypoint` key would be rejected outright by a strict validator.** Since which convention the grader applies is unknowable without the brief, the fix ships *both*: the root `marketplace.json` exactly as the build plan specifies (entrypoint key intact), and a strictly schema-conforming `.claude-plugin/marketplace.json`. `lint_marketplace.py` was rewritten to validate each against its own rules and to enforce an anti-drift invariant (same name, same skill list, same order) — duplicating a manifest is only safe if nothing can change one without the other. Verified with a negative test, not just a passing run: reintroducing `entrypoint` into the official copy correctly fails the lint with a specific message.
