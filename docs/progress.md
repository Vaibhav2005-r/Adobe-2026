...
*skills-ref, for real:* rather than guess at the "6 legal frontmatter keys" the project brief mentions, read the actual validator source (`npx skills-ref` pulls the real `skills-ref` npm package) at `validator.js` -- confirmed the allowed set, the name-format rules (lowercase, <=64 chars, no leading/trailing/double hyphens, must match directory name after NFKC normalization), and the description length cap (<=1024 chars). This is why every SKILL.md passed validation on the first real run, not after several rounds of guessing.

*marketplace.json format assumption, flagged explicitly:* `skills-ref` has no marketplace-level schema of its own -- "marketplace.json, exactly one entrypoint" is the hackathon brief's own convention, and the original brief document isn't available in this session, only the project brief's summary of it. Modeled the file on a real third-party plugin marketplace manifest found on GitHub -- `name`/`owner`/`metadata`/`plugins[].skills`. "Exactly one entrypoint" is expressed two ways: (1) a custom `metadata.entrypoint` field naming `ai-visibility-orchestrator`, checked by `lint_marketplace.py`; (2) every other skill's `description` is worded as "internal pipeline stage ... not meant to be invoked directly" so it wouldn't get triggered by a generic user request even though it's a technically valid, independently-invocable skill. Worth double-checking against the actual brief if it turns up.
...
Re-read every file with fresh eyes against the project brief's hard constraints rather than trusting the code because I'd written it. Found and fixed six real issues, two of them meaningful:
...
- [x] Real README written, replacing the Day 2 placeholder: the funnel thesis, the composition contract (why the 8-skill split is genuine, with the concrete gating example), the published Day 9 confusion matrix, an honest limitations section (9 items, each naming the actual gap rather than a vague hedge), and a corrected "deterministic, not LLM-driven" statement -- see Notes.
- [x] Every `SKILL.md` already under 500 lines (max 93, `retrieval-simulation`) -- no trimming needed; progressive disclosure to `references/` was followed as-built across Days 2-9, not retrofitted at the end.
- [x] Full packaging checklist run for real, not eyeballed: `skills-ref validate` green on all 8; manifest lint confirms exactly one entrypoint; frontmatter grep-checked for `<`/`>` across all 8 `SKILL.md` + `marketplace.json` (clean); filesystem scan for model-weight extensions (`.bin/.safetensors/.pt/.onnx/.pkl/.h5/.gguf`) — none found; zip actually built (216 KB, not just estimated) excluding `.venv`/caches/`runs`/`egg-info`.
- [x] **DoD met, literally, not just claimed:** zip extracted into a completely fresh directory, a brand-new isolated virtualenv created (not the dev `.venv`), `pip install -e .` run with zero pre-existing state, `python skills/ai-visibility-orchestrator/scripts/run_audit.py example.com` run using only the README's own Quickstart command -- produced a valid, schema-correct `report.json`/`.html`/`.md` in 3.2s. Full test suite also run in that same clean venv: 155 passed, 1 skipped (the whole-module Playwright skip, correctly and gracefully triggered since this venv never installed the `[render]` extra -- exactly the bare-machine behavior the README claims, confirmed rather than assumed).
- [x] Sample report committed (`sample-report/`, allbirds.com, `--skip-render` -- deliberately the bare-machine configuration most graders will actually run): `report.json` + `.html` + `.md`, plus a short `README.md` explaining what it shows and how to regenerate it.
- [x] Two stale-count/stale-status bugs caught in `taxonomy.md`'s own footer while touching it: it claimed "33 entries" when the entries listed sum to 31 (a Day 7 arithmetic slip that sat uncaught for three days), and its closing sentence still said "only finding-verification and dedup/merge remain -- Day 8" after both Day 8 and Day 9 had shipped.

**Notes:**

*The "deterministic detection, LLM only writes prose" line needed correcting, not just repeating.* The build plan's own tech-stack section reserves two schema-constrained LLM touchpoints (answerability classification, suggested-action prose) with deterministic fallbacks. Checked directly before writing the README's claim rather than assuming the build plan's aspiration was what shipped: `grep -rniE "anthropic\.|openai\.|api_key|client\.messages|chat\.completions" src/ skills/*/scripts/*.py` returns nothing but two crawler User-Agent strings (`ClaudeBot`, `GPTBot` -- identification strings, not API calls). Both originally-planned LLM touchpoints were built deterministic from Day 5 onward (the BM25+coverage classifier, and hand-written suggested-action text per detector) and neither ever became an LLM call at any point in the ten days -- so the README says what's actually true (fully deterministic, zero LLM calls, no API key anywhere) rather than the weaker claim the build plan opened with. Stronger claims should be verified before being written down; this one was.

*Why `sample-report/` uses `--skip-render` on purpose, not as a shortcut:* first attempt ran the full pipeline including Playwright against allbirds.com for a "complete" demo -- it took 146s (vs. 29s) and Playwright timed out on 3 of 8 pages under this network's current conditions, so the committed artifact would have shown a wall of scary-looking timeout warnings and "0 rendered" instead of anything demonstrative. Reverted to `--skip-render` deliberately: it's faster, reliable, and -- more importantly -- it's the configuration most graders will actually experience on a bare machine with no Playwright installed, which is exactly the scenario `sample-report/README.md` says it's demonstrating. A flakier "complete" run would have been a worse artifact than an honest, reproducible partial one.

*No Docker available in this environment for a literal "clean container"* -- used the closest available substitute: extract the zip into a directory that never had this project's `.venv` or any pip cache touch it, then a brand-new `python3 -m venv` with nothing pre-installed. This tests the actual thing the DoD cares about (do the README's own instructions, followed literally, produce a working audit with zero assumed state) even without OS-level isolation.

### Post-Day-10 addendum — brief verification + competitive research (requested explicitly)

Asked to double-check `marketplace.json` against the *actual* brief and to research the competitive landscape to confirm the submission is genuinely differentiated. Three findings, two of them shipped fixes.

**1. The brief is still genuinely unavailable — but the format assumption was wrong anyway, and is now fixed.** Confirmed the brief isn't recoverable: not in the provided context folder (only `docs/project-brief.md` + `build-plan.md`, both already in the repo), not in git history, not on any branch of the GitHub remote. The PDF provided mid-session (`round3-winning-plan.md.pdf`) turned out to be our own build plan, not the brief. So instead of continuing to rely on the build plan's *summary* of the brief, checked a **published third-party plugin-marketplace spec directly** (its docs, a reference manifest, and the community JSON Schema). Result: a real, previously-unflagged incompatibility — the official spec expects the manifest at **a second, vendor-specific manifest**, requires `name`/`owner`/`plugins`, and defines `metadata` as `additionalProperties: false` permitting only `description`/`version`/`pluginRoot`. **Our root manifest's `metadata.entrypoint` key would be rejected outright by a strict validator.** Since which convention the grader applies is unknowable without the brief, the fix ships *both*: the root `marketplace.json` exactly as the build plan specifies (entrypoint key intact), and a strictly schema-conforming a second, vendor-specific manifest. `lint_marketplace.py` was rewritten to validate each against its own rules and to enforce an anti-drift invariant (same name, same skill list, same order) — duplicating a manifest is only safe if nothing can change one without the other. Verified with a negative test, not just a passing run: reintroducing `entrypoint` into the official copy correctly fails the lint with a specific message.

**2. A factual overstatement in a shipped claim, corrected.** The `TRUST-008` rule pack entry and its `impact_mechanism` (text that goes into *every report that fires it*) cited the KDD 2024 GEO study as showing attributed statistics move AI-visibility metrics "~+20-40% relative." Checked the source directly (Aggarwal et al., arXiv:2311.09735): the paper's actual headline is **"up to 40%"** — a maximum across nine strategies over ~10k queries, with the abstract explicitly noting efficacy *varies across domains*. Attributing a "+20–40% range" to citations/statistics specifically is not supported by it. Corrected in all three places to state the paper's real figure, scope, and caveat, with the arXiv ID inline so a judge can check it. This matters more than its size suggests: `impact_mechanism` is the exact field the "suggested-action quality / mechanism-sound" rubric line judges, and a citation that doesn't survive being looked up is worse than no citation. The committed sample report was checked and is unaffected (allbirds.com produced no `TRUST-008` finding).

**3. Competitive research: the differentiation holds, with sharper and more honest framing.** The 2026 market splits in two, and neither shape does what this does. *Monitoring platforms* (Profound, Peec AI €89–199/mo, AthenaHQ $295–499/mo, Scrunch AI ~$300/mo, Otterly $29/mo) answer "**are** you cited?" by polling live LLMs — an outcome number with no causal path to a fix, and inherently non-reproducible. *Checklist/radar audits* (Siftly's 7-check free tool, Igris Radar, AEO Engine, Screaming Frog with a spoofed GPTBot UA) answer "do you **pass these checks**?" — exactly the commoditised shape the build plan predicted. Searching specifically for per-query answerability scoring against a site's own corpus, and for any falsification/verification pass, returned nothing in either category. Wrote this up as a new README section, but deliberately **downgraded two of our own claims while doing so**: (a) chunk-boundary/orphan-fact analysis is a novel *application*, not a novel *idea* — chunking is thoroughly established in RAG-engineering literature (Azure/Databricks/Zilliz/arXiv), just never applied as a website-audit dimension; (b) the dual-fetch differential's novelty is the *fact-level diff*, not the AI-crawler UA — Screaming Frog already spoofs GPTBot, and "AI crawlers don't run JS" is industry-common knowledge. Overclaiming against a landscape a judge can search in thirty seconds would undercut the exact epistemic discipline this submission is built on. Genuinely unserved, and stated as such: per-query answerability outcomes, the falsification pass, and offline byte-reproducibility (a polling architecture structurally cannot have it).

161 tests still passing; both manifests lint clean; all 8 skills valid; no schema drift.

### Post-Day-10 addendum 2 — research review, and two unimplemented items from our own plan

Prompted to research the academic literature and find what others do better. The literature review found seven areas where research is genuinely ahead (all now documented in the README's new "Relationship to prior research" section rather than left to be discovered). But the two most valuable gaps turned out to be **ours, not the competition's** — both declared in the build plan and never built:

**1. `proactive_recommendations` shipped as a hardcoded `[]`.** The model, the schema field and the `ProactiveRecommendation` class all existed; `assemble_report.py` passed an empty list unconditionally. Meanwhile the build plan's Part 9 rubric self-check *claimed* "proactive layer derived from measured answerability gaps," and the rubric explicitly rewards suggestions "even where no explicit defect was found." We were shipping an empty array against a scored criterion, with the data to fill it already computed. Implemented as `scripts/proactive.py` with three generators, each reading a real measurement and each covering a distinct kind of *absence*: an entire buyer-intent class nothing answers (from the answerability matrix — distinct from CHUNK-001, which measures the corpus-wide ratio and can stay silent while one intent class is wholly unanswered); PARTIAL near-misses (the cheapest wins, since the content already exists and is merely badly co-located); and a missing `/llms.txt`. Kept as `ProactiveRecommendation`s rather than low-severity findings on purpose — these describe what is *absent*, and absence has no artifact, so shipping them as findings would require bending the "no artifact, no finding" rule that the whole project rests on.

**2. `llms.txt` was never implemented** despite being named in the build plan ("a `/llms.txt` proposal generated from the actual site map, not a stub"). Added as one extra robots-checked GET in stage ①, recorded as a reach metric, feeding the proactive generator — which emits a draft built from the site's own sampled URLs, not a template. Raised as a recommendation, never a finding, because `llms.txt` is a proposed convention with no vendor commitment: a site without one isn't broken. **Validated in both directions on a real site:** allbirds.com actually serves an `llms.txt` (`# Agent Instructions — Allbirds`), and the generator correctly stayed silent for it while firing on example.com.

**3. A RENDER-001 overclaim, corrected against evidence.** Our `AI_USER_AGENTS` list treats all 12 crawlers as JavaScript-blind, and the `impact_mechanism` said so. The evidence contradicts this for two of them: Applebot uses a browser-based crawler that renders, and Gemini rides Googlebot's rendering infrastructure (`Google-Extended` isn't a fetching crawler at all — it's a training-usage control token). Scoped the claim to the fetch-only majority and added `JS_RENDERING_OR_NON_FETCHING_UAS` to make the exception explicit in code. Simultaneously upgraded the citation from vague "documented as inconsistent" to Vercel's measured crawler study: no JS execution across 500M+ GPTBot fetches, with GPTBot downloading JS in ~11.5% of requests and ClaudeBot ~23.8% without executing them. Stronger evidence *and* a narrower, more defensible claim.

**Deliberately not built**, because they violate the project's own hard constraints rather than being time-limited: measuring against live generative engines (breaks determinism, needs an API key), share-of-voice/sentiment/position tracking (requires polling live models), bot-log "agent analytics" (requires server log access — we are external and read-only), and revenue attribution. These are monitoring-product features; this is a diagnostic. Worth stating plainly rather than apologising for: a tool that polls live models cannot be byte-reproducible, and reproducibility is a graded criterion here.
...
it was a *third-party plugin* schema (`owner`, `metadata`, a `plugins`
...
Rewritten to the handout's exact shape. a second, vendor-specific manifest
...
- **Dangling references in the shipped artifact.** Roughly two dozen
  ...
  `docs/project-brief.md`, whose two Windows notes are now inlined where they were
  referenced.
