...
Day 9 (Sat 5 Sep): evaluation & precision hardening — done. Built a fixture eval harness (`scripts/eval_fixtures.py`) and published real numbers: **precision 1.00, recall 1.00, FP-rate on clean controls 0.00**. Getting there found two real, previously-shipped bugs: `REACH-001` was flagging a robots.txt exclusion of one low-value page (blocked for *every* crawler, not AI-specifically) as if it were AI-targeted discrimination — fixed by checking whether a generic non-AI crawler would also be blocked. A wild-corpus run against zalando.de crashed the pipeline outright (a pydantic `ValidationError` — `arrive_detect.detect_no_ai_referral_instrumentation` built a Finding with zero artifacts when the whole crawl came back empty); fixed with the same empty-corpus guard `retrieve_detect` already needed on Day 5, now correctly propagated to the detector that was missing it. Determinism strengthened from two runs to three per the DoD's literal wording. 161 tests passing. See `docs/progress.md` for the full accounting, including a flagged assumption about `marketplace.json`'s exact format (the original hackathon brief isn't available in-session, only this file's summary of it — worth double-checking). Update this section (or better, ask me to log it in `docs/progress.md`) as each day's DoD from the build plan is met, so a fresh session picks up where the last one left off.
