...
- **Source:** nytimes.com; live query "is New York Times free to read or subscription required" → 0/9 returned links were nytimes.com; robots.txt confirmed `Disallow: /` for 9 named AI bots including GPTBot and ClaudeBot. The AI-specific-targeting check was added Day 9 after the automated detector false-positived on a compliance fixture's ordinary `User-agent: *` page exclusion — recorded in the development log.
...
- **Two guards added after a real false positive on thesouledstore.com,** where this rule was the report's *headline* finding and was wrong on both counts:
  - **Corroboration is checked against bare numerals too, not only symbol-prefixed ones.** `_CURRENCY_RE` requires a currency symbol adjacent to the digits, but a very common way to write prices does not: a table with a `Price (INR)` column header states every value as a bare number, factoring the symbol out into the header exactly once. The page's own main content read `Harry Potter: Potter Chibi | 499` while the finding claimed JSON-LD's `499` matched no visible price. The looser bare-number set is only ever consulted to *suppress* a contradiction claim, never to raise one — a bare number that isn't really a price can only cost a finding, never manufacture one — and the narrow symbol-anchored set is still what gets reported as evidence.
  - **The rule does not run on listing pages.** Its mechanism is a *single* product's schema price disagreeing with that same product's visible price. A collection page carrying one `Offer` per product in a grid (24 of them, in the case found) gives the detector no way to tell which of twenty visible prices belongs to which Offer, so "does this price appear anywhere on the page" is answerable by coincidence in both directions. Above four distinct prices, the page is treated as a listing and skipped.
- **Source:** engineering-derived from the build plan's Day 4 requirement (a named example: "JSON-LD says `price: 199`, page says `₹24,999`"); not yet observed as a true positive on a real site during field research — the one real-site firing to date was the false positive above.
...
- **False negative caught later, by external code review:** the first implementation compared each heading against a *running maximum* rather than the previous heading. A running maximum never decreases, but a document outline does -- `h1 > h2 > h3 > h2 > h3` is correct HTML, and every sibling section legitimately resets the working depth. So one deep heading early in a page raised the bar for everything after it: following an `h4` in section 1, a section 2 that went `h2 > h5` passed (`5 > 4 + 1` is false) and the skip went unreported. Every skip after a page's first deep heading was masked the same way. Fixed to compare against the preceding heading; regression-tested with the masked sequence, with a correct descending outline (must stay silent), and with two skips on one page (both must be reported).
...
- **Language scope:** the query bank and the BM25 stopword list are English-only, so the probe **does not run at all** when the corpus declares a language they don't cover; stage ④ then reports `skipped`, the answerability matrix is empty, and the degradation names the language (see `brand_audit.language`). Measured, not assumed: a deliberately well-built German fixture (brand named up top on every page, JSON-LD, prices in both schema and prose) scored 15/18 unanswerable, with the pricing page's own sentence `Der Bergquell A1 Aktivkohlefilter kostet 149,00 EUR` -- a verbatim, attributable answer -- classified UNGROUNDED. Only the three identity queries passed, and only because a brand name is the one token that survives translation. Suppressing the probe is the same contract as a missing Playwright: skip the measurement, suppress the findings, record the degradation, never guess.
...
- **Detection method:** parse `dateModified`/`datePublished` from JSON-LD as ISO 8601; compare against a reference date (real "now" in production, parameterized for testability); flag if the gap exceeds 365 days. Scans the **whole corpus** and reports once, aggregated: the oldest over-threshold date per page, then a single finding scoped `checked=len(pages), affected=len(stale_pages)`. Returning on the first stale page found (the original implementation) reported `checked=1, affected=1` however much of the site was stale, which both understated the problem to the reader and starved the severity function and the sample-adequacy check of the scope they compute from.
- **Evidence artifact:** the property name, its raw value, and the computed age in days, for the five oldest pages.
- **Severity default:** medium; `high` when a majority of the corpus is stale, since a mostly-unreviewed site is a page-class problem rather than a local one.
...
- **Source:** engineering-derived from the build plan's Day 7 requirement ("answer proximity... present in text above the fold, or buried below three hero sections"); reuses position data stage ④ already computes rather than re-deriving it — see the char-offset regression fix noted in the development log's Day 7 entry.
...
- **Language scope:** the phrase list is English-only, so this detector is **suppressed entirely** when the corpus declares a language the bundled lexicons don't cover (see `brand_audit.language`). On a German fixture it reported "no recognizable next step" for pages whose next step was a `Jetzt bestellen` link. Same contract as a missing Playwright: skip the measurement, suppress the findings, record the degradation, never guess.
- **Source:** engineering-derived from the build plan's Day 7 requirement ("next-step availability... not behind a navigation hunt"); observed firing at low confidence on a real non-commercial site during the Day 7 sweep (docs.python.org's version-index page, where "no CTA" is arguably an accurate but low-stakes finding for a documentation site rather than a commercial one — recorded honestly rather than suppressed, recorded in the development log).
